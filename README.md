...
| `src/NodeWalk.lua` | Walkability by a brick test against the voxel NODES rather than the parts, then regions. Also normalizes union and mesh faces: fits each face from the surface, resamples it onto a regular grid in its own plane, and hands Contour something with a lattice. |
